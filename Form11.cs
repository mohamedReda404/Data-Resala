    ...
    public partial class Form11 : Form
    {
        ...
        private void SaveData(string name, string status, string nationalID, string maritalStatus, string phone1, string phone2, string area, string address, string explorationDate, string evaluationDate, string evaluation)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "INSERT INTO table_11t (الاسم,الحالة,الرقم_القومي,الحالة_الاجتماعية,الهاتف_1,الهاتف_2,المنطقة,العنوان_بالتفصيل,تاريخ_الاستكشاف,تاريخ_التقييم,التقييم) "
        ...
        private void LoadData()
        {
            try
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT * FROM t11"; // Query to get all data
                                      ...
                                      string explorationDate, string evaluationDate, string evaluation)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT * FROM table_11t WHERE 1 = 1"; // استخدام 1 = 1 كقاعدة ثابتة لجعل شروط البحث مرنة
                             ...
                             string explorationDate, string evaluationDate, string evaluation)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "UPDATE table_11t SET الاسم = @Name, الحالة = @Status, الحالة_الاجتماعية = @MaritalStatus, " +
        ...
        private void DeleteData(string nationalID)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM table_11t WHERE الرقم_القومي = @NationalID";
