    ...
    public partial class Form4 : Form
    {
        ...
        private void SaveData(string name, string status, string nationalID, string maritalStatus, string phone1, string phone2, string area, string address, string explorationDate, string evaluationDate, string evaluation)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "INSERT INTO tb_4 (الاسم,الحالة,الرقم_القومي,الحالة_الاجتماعية,الهاتف_1,الهاتف_2,المنطقة,العنوان_بالتفصيل,تاريخ_الاستكشاف,تاريخ_التقييم,التقييم) "
        ...
        private void LoadData()
        {
            try
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT * FROM tb_4"; // Query to get all data
                                      ...
                                      string explorationDate, string evaluationDate, string evaluation)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT * FROM tb_4 WHERE 1 = 1"; // استخدام 1 = 1 كقاعدة ثابتة لجعل شروط البحث مرنة
        ...
        private void DeleteData(string nationalID)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "DELETE FROM tb_4 WHERE الرقم_القومي = @NationalID";
                             ...
                             string explorationDate, string evaluationDate, string evaluation)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string query = "UPDATE tb_4 SET الاسم = @Name, الحالة = @Status, الحالة_الاجتماعية = @MaritalStatus, " +
